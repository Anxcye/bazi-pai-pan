...
import type { BaziChartResult, LuckSequenceItem, PillarDetail } from '../../domain/bazi/types.ts'
...
export function ChartPreviewSection({ result, config }: ChartPreviewSectionProps) {
  ...
  return (
    <div className="space-y-6">
                      ...
                      {pillars.map((pillar) => (
                        <td
                          ...
                          className={cn(
                            ...
                            pillar.key !== 'hour' && 'border-r border-stone-300 dark:border-white/15',
                            row.emphasis === 'stem' && 'py-5 text-[2rem] font-semibold leading-none tracking-[0.2em] text-stone-950 dark:text-stone-50',
                            row.emphasis === 'branch' && 'py-5 text-[2rem] font-semibold leading-none tracking-[0.2em] text-stone-950 dark:text-stone-50',
      ...
      <LuckRow title="大运" items={result.dayun} emptyLabel="暂无大运" />
      <LuckRow title="流年" items={result.liunian} emptyLabel="暂无流年" />
      <LuckRow title="流月" items={result.liuyue} emptyLabel="暂无流月" compact />
      <LuckRow title="流日" items={result.liuri} emptyLabel="暂无流日" compact />
      <LuckRow title="流时" items={result.liushi} emptyLabel="暂无流时" compact />
...
function LuckRow({ title, items, emptyLabel, compact = false }: { title: string; items: LuckSequenceItem[]; emptyLabel: string; compact?: boolean }) {
  return (
    <section className="rounded-[18px] border border-stone-300 bg-white p-5 shadow-[0_10px_26px_-24px_rgba(68,53,35,0.4)] dark:border-white/12 dark:bg-stone-950">
      <h3 className="text-base font-semibold text-stone-950 dark:text-stone-50">{title}</h3>
      {items.length === 0 ? (
        <div className="mt-4 text-sm text-stone-500 dark:text-stone-400">{emptyLabel}</div>
      ) : (
